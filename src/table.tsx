...
// Due to some import issues, I've copied the code here and modified it to work with Bun.
...
export type TableProps<T extends ScalarDict> = {
  ...
  /**
   * Whether to show row separators between data rows.
   */
...
> {
  ...
  getConfig(): TableProps<T> {
    return {
      ...
      showSeparators: this.props.showSeparators ?? true,
    }
  }
  ...
  render() {
    ...
    const { showSeparators } = this.getConfig()
    ...
    return (
      <Box flexDirection="column">
        ...
        {this.props.data.map((row, index) => {
          ...
          // Construct a row.
          return (
            <Box flexDirection="column" key={key} width="100%">
              {showSeparators && this.separator({ key: `separator-${key}`, columns, data: {} })}
